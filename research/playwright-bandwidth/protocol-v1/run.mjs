...
import { parseArgs, validateRows, sameRows, aggregate, isKnownBrowserPolicyBlock } from './core.mjs';
...
try {
  ...
  manifest = { protocol: '1.0', started_at: new Date().toISOString(), status: 'running', node_version: process.version, playwright_version: pkg.version, browser_channel: process.env.BROWSER_CHANNEL || 'bundled-chromium', platform: process.platform, architecture: process.arch, target: 'https://books.toscrape.com', pages: args.pages, repeats: args.repeats, expected_records_per_page: 20, concurrency: 1, gap_ms: 300, timeout_ms: 60000, retries: 0, fresh_context_per_navigation: true, service_workers: 'block', routing_disables_cache_in_both_arms: true, lean_blocked_types: ['image', 'font', 'media'], known_browser_policy_block: { url: 'http://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js', failure: 'mixed-content', handling: 'excluded from unexpected failures; browser security unchanged in both arms; no transfer bytes recorded' }, order: 'full/lean on even pair index; lean/full on odd pair index; index continues across repetitions', proxy_used: true, proxy_identity: 'omitted', viewport: { width: 1440, height: 900 }, locale: 'en-GB', timezone: 'UTC', billed_bytes: null };
  ...
  for (let repeat = 1; repeat <= args.repeats; repeat++) {
    for (let number = 1; number <= args.pages; number++) {
      ...
      for (const arm of index % 2 === 0 ? ['full', 'lean'] : ['lean', 'full']) {
        ...
        const stat = { repeat, page: number, arm, started_at: new Date().toISOString(), observed_bytes: 0, completed_requests: 0, failed_requests: 0, browser_policy_blocked_requests: 0, browser_policy_block_reasons: {}, intentionally_blocked_requests: 0, missing_sizes: 0, request_types: {}, statuses: {}, rows: [], validation_errors: [] };
        ...
        } catch (error) { stat.validation_errors.push(error.name === 'TimeoutError' ? 'timeout' : 'navigation_or_extraction_error'); }
        ...
        console.log(JSON.stringify({ repeat, page: number, arm, records: stat.rows.length, bytes: stat.observed_bytes, errors: stat.validation_errors }));
  ...
  if (manifest) {
    ...
    await save('manifest.json', manifest);
    await save('summary.json', aggregate(pages));
